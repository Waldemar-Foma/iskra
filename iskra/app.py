...
class AgentSimulator:
    ...
    def _check_pending_dialogues(self):
        ...
        if self.pending_dialogues:
            print(f"🔍 Проверяю {len(self.pending_dialogues)} ожидающих диалогов...")
            ...
            if result:
                ...
                try:
                    if pending.get('type') == 'response':
                        ...
                        # Сохраняем в историю диалога в GigaChatManager
                        ...
                        # Обычный диалог (первое сообщение)
                        ...
                        # Обновляем отношения
                        agent = Agent.query.get(pending['agent_id'])
                        target = Agent.query.get(pending['target_id'])
                        if agent and target:
                            self._update_relationship(agent, target, change=0.1)
                ...
                if user_message:
                    ...
                    user_message.sender_type = 'agent'
                    ...
                    # Создаем событие
                    event = Event(
                        event_text=f"💬 Пользователь {pending.get('username')} общается с {pending['agent_name']}",
                        agent1=pending['agent_name'],
                        agent2=None,
                        event_type='человек-агент',
                        world_cycle=pending['world_cycle']
                    )
                    db.session.add(event)
                ...
                if pending['attempts'] % 5 == 0 and pending['attempts'] > 0:
                    print(f"⏳ Задача {task_id} еще не готова (попытка {pending['attempts']})")
                
                if pending['attempts'] > 20:
                    print(f"⏰ Таймаут задачи {task_id}")
                    completed.append(task_id)
...
# API для отправки сообщения агенту
@app.route('/api/chat/send', methods=['POST'])
@login_required
def send_chat_message():
    """Отправка сообщения агенту от пользователя с проверкой ограничений"""
    user = User.query.get(session['user_id'])
    ...
    if len(message) > 500:
        return jsonify({'success': False, 'error': 'Сообщение слишком длинное (макс. 500 символов)'}), 400
    
    agent = Agent.query.get(agent_id)
    if not agent:
        return jsonify({'success': False, 'error': 'Агент не найден'}), 404
    
    # ОГРАНИЧЕНИЯ ПО ТИПАМ АГЕНТОВ И УРОВНЮ ПОДПИСКИ
    subscription_tier = user.subscription_tier or 'basic'
    
    # Проверка доступа к агентам разных типов
    if agent.type == 'Продвинутая' and subscription_tier not in ['premium', 'vip']:
        return jsonify({
            'success': False, 
            'error': 'Продвинутые агенты доступны только с подпиской Premium или VIP'
        }), 403
    
    if agent.type == 'Бесконечная' and subscription_tier != 'vip':
        return jsonify({
            'success': False, 
            'error': 'Агенты "Бесконечная" доступны только с VIP-подпиской'
        }), 403
    ...
    limits = {
        'basic': 20,     # 20 сообщений в день
        'premium': 100,  # 100 сообщений в день
        'vip': 500       # 500 сообщений в день
    }
    ...
    if messages_today >= daily_limit:
        return jsonify({
            'success': False, 
            'error': f'Достигнут дневной лимит сообщений ({daily_limit}). Обновите подписку для увеличения лимита'
        }), 429
    
    # Проверка кулдауна между сообщениями (не чаще чем раз в 10 секунд)
    last_message = UserAgentChat.query.filter_by(
        user_id=user.id,
        sender_type='user'
    ).order_by(UserAgentChat.timestamp.desc()).first()
    
    if last_message:
        time_since_last = (datetime.utcnow() - last_message.timestamp).seconds
        if time_since_last < 10:
            return jsonify({
                'success': False, 
                'error': f'Подождите {10 - time_since_last} секунд перед следующим сообщением'
            }), 429
    ...
    # Получаем контекст для GigaChat
    world = WorldState.query.first()
    
    # Формируем контекст с учетом того, что это человек пишет агенту
    context = {
    ...
    if task_id:
        # Сохраняем связь задачи с диалогом
        ...
        # Если агент на кулдауне
        return jsonify({
            'success': False, 
            'error': 'Агент временно недоступен. Попробуйте позже.'
        }), 429
...
def check_response(conversation_id):
    """Проверка, получен ли ответ от агента"""
    user = User.query.get(session['user_id'])
    
    # Ищем ответ от агента
    agent_message = UserAgentChat.query.filter_by(
        user_id=user.id,
        conversation_id=conversation_id,
        sender_type='agent',
        response_received=True
    ...
    if agent_message:
        return jsonify({
            'response_received': True,
            'message': agent_message.response,
            'agent_name': agent_message.agent.name
        })
